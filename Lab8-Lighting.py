""" Modified code from Peter Colling Ridge
	Original found at http://www.petercollingridge.co.uk/pygame-d-graphics-tutorial
"""
...
class WireframeViewer(wf.WireframeGroup):
    ...
    def __init__(self, width, height, name="Wireframe Viewer"):
        ...
        self.eyeY = 100
        self.light_color = np.array([1,1,1])  #THESE ARE IMPORTANT
        self.view_vector = np.array([0, 0, -1])  #your eye
        self.light_vector = np.array([0, 0, -1])  #light soure
            ...
            if self.displayFaces:
                for (face, colour) in wireframe.sortedFaces():
                    ...
                    if towards_us > 0:
                        ...
                        #do phong model and set light total!! keep between 0-255, dont display if negative
                        s = self.light_color
                        m_diff = m_diff * colour
                        n = normal

                        vec = np.multiply(s, m_diff)
                        scal = np.dot(n, self.light_vector)

                        c_diff = vec * scal

                        m_spec = m_spec * colour
                        v = self.view_vector
                        m_gls = 4
                        addLight = True
                        if (scal <= 0):
                        ...
                        r = 2 * scal * n
                        r = r - np.array(self.light_vector)
                        first = np.multiply(s, m_spec)
                        second = np.dot(v, r)
                        if (second <= 0):
                            c_spec = np.array([0, 0, 0])
                        else:
                            second = second**4
                            c_spec = first * second

                        if (addLight):
                            light_total = c_diff + c_spec + ambient

                        else:
                            light_total = ambient

                        np.clip(light_total, 0, 255)
    ...
    def keyEvent(self, key):
        ...
        #can use self, modify light vector by rotation matrix
        #Your code here
        if key == pygame.K_w:
            a = np.array(([1, 0, 0],
                [0, math.cos(math.radians(-3)), -math.sin(math.radians(-3))],
                [0, math.sin(math.radians(-3)), math.cos(math.radians(-3))]))
            b = np.array(self.light_vector)
            self.light_vector = a.dot(b)
        ...
        if key == pygame.K_a:
            a = np.array(([math.cos(math.radians(-3)), 0, -math.sin(math.radians(-3))],
             [0, 1, 0],
             [math.sin(math.radians(-3)), 0, math.cos(math.radians(-3))]))
            b = np.array(self.light_vector)
            self.light_vector = a.dot(b)
        ...
        if key == pygame.K_q:
            a = np.array(([math.cos(math.radians(-3)), -math.sin(math.radians(-3)), 0],
             [math.sin(math.radians(-3)), math.cos(math.radians(-3)), 0],
             [0, 0, 1]))
            b = np.array(self.light_vector)
            self.light_vector = a.dot(b)
